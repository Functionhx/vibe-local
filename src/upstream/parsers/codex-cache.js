...
// MODIFIED FROM UPSTREAM — see src/upstream/UPSTREAM.md §差异 1
//
// Upstream defaults to ~/.vibe-usage/cache and honours VIBE_USAGE_CACHE_DIR.
// Both are changed here: this tool must not read or write the real
// vibe-usage's cache directory, and inheriting its env var would reintroduce
// exactly that collision for anyone who has it set. Ours is a separate name at
// a separate path, so the two tools cannot interfere with each other even when
// installed side by side.
export function codexCacheDir(codexHome) {
  const base = process.env.VIBE_LOCAL_CACHE_DIR?.trim()
    || join(homedir(), '.vibe-local', 'cache');
